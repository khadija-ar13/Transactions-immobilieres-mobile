...
export default function ({ navigation }) {
  ...
  const fetchAnnouncementsByFilter = async (filterQuery) => {
    try {
      const response = await fetch("http://192.168.57.43:3002/annonces/filtre_annonce_publiee_mobile", {
  ...
  const fetchCommuneSuggestions = async (query) => {
    try {
      const response = await fetch(
        `http://192.168.57.43:3002/communes/search/${query}`
      );
  ...
  const fetchAnnouncements = async () => {
    try {
      const response = await fetch("http://192.168.57.43:3002/annonces/Annonces_publie_include_annonceurid");
      let data = await response.json();
